...
<div className="flex items-center max-w-sm mx-auto">   
    ...
    <button type="submit" onClick={() => SearchIterate(notes, setFiltered, search, noteData, todoData)} className="p-2.5 ms-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
        <svg className={`w-4 ${!filtered.length ? "block" : "hidden"} h-4`} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"/>
        </svg>
<span onClick={() => setFiltered([])} className={`w-4 ${filtered.length ? "block" : "hidden"} search h-4 flex items-center justify-center text-xl`}><TbZoomCancel /></span>
